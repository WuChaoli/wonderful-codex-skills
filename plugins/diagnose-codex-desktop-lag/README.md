...
[![CI](https://github.com/WuChaoli/wonderful-codex-skills/actions/workflows/validate.yml/badge.svg)](https://github.com/WuChaoli/wonderful-codex-skills/actions/workflows/validate.yml)

## 描述 / Description

用于诊断 Windows、macOS 或 Linux 上 Codex Desktop 的卡顿、冻结、输入延迟、高 CPU/GPU/内存/I/O、SQLite WAL 增长、子进程频繁启动、会话历史膨胀和窗口渲染异常。

Diagnose Codex Desktop freezes, stutter, high resource usage, SQLite logging pressure, child-process churn, large local history, and rendering problems on Windows, macOS, or Linux.

Skill 先逐项运行 18 个独立只读 Node.js 诊断命令并生成完整报告。修复阶段每次只处理一个已确认问题：先用 `--what-if` 预览，再次确认后使用 `--apply`，并立即复测。
...
优先使用修复命令输出的时间戳备份目录。涉及数据库或会话归档时，完全退出 Codex 后运行 `node scripts/codex-performance.mjs remediate restore-codex-backup --what-if`，确认预览后再加 `--apply`，并重新执行数据库完整性和任务状态诊断。
